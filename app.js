...
app.get("/Limitmusic", function (req, res) {
    conn.query(`SELECT * FROM MusicData LIMIT 10`, [], function (err, result) {
        res.send(result)
    })
})
//只取20筆----
...
app.get('/channel/Album/:id', function (req, res) {
    var data = req.params.id
    conn.query(`SELECT DISTINCT AlbumData.AlbumID,AlbumData.AlbumName,AlbumData.AlbumImage,AlbumData.AlbumTime FROM AlbumData,MusicData WHERE MusicData.MusicID = AlbumData.MusicID AND MusicData.UserID = ?;`, [data], function (err, result) {
        res.send(result)
    })
})
//專輯所有歌曲------
...
app.get("/channelBlog/:channelID/blog/:blogID", function (req, res) {
    ...
    conn.query(`SELECT * FROM BlogData WHERE ChannelID = ? AND BlogID != ?`, data, function (err, result) {
        res.send(result)
    })
})
...
app.get("/blogOtherPage/:channelID/blog/other/:blogID", function (req, res) {
    var data = [req.params.channelID, req.params.blogID]
    conn.query(`SELECT * FROM BlogData WHERE ChannelID != ? AND BlogID !=? LIMIT 3`, data, function (err, result) {
        res.send(result)
    })
})
...
// singer name
app.get("/search/singer/:id", function (req, res) {
    var data = [req.params.id]
...
app.post("/signup", function (req, res) {
    var { UserMail, UserPW, UserName, UserBrith } = req.body;
    UserPW = bcrypt.hashSync(UserPW, 10);
    console.log(req.body);
    conn.query(
        `INSERT INTO userinfo (UserID, UserMail, UserPW, UserName, UserSex, UserPhone, UserBrith)  VALUES ('1', '${UserMail}', '${UserPW}', '${UserName}', '', 'NULL', '${UserBrith}')`,
